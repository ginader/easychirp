...
if (count($saved_searches) == 0) {
	...
	foreach($saved_searches AS $saved):
		echo '<li><a href="/search_results?query=' . urlencode($saved->query) . '">' . $saved->name . '</a> <a href="#?id=' . $saved->id . '" class="delete-link delete-search"><span aria-hidden="true" class="icon-close1"></span> ' . $xliff_reader->get('global-delete') . '</a></li>';
	endforeach;
	echo '</ul>';
}
?>
